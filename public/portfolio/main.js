...
(function () {
  ...
  function renderProjects() {
    ...
    S.projects.forEach(function (p) {
      ...
      if (p.image) {
        var shot = el("div", "card__shot" + (p.fit === "contain" ? " card__shot--contain" : ""));
        var img = el("img");
        img.src = p.image;
      ...
      // No caption on the card — the thumbnail carries it. title and
      // description stay in content.js and become the link's accessible
      // name above, so screen readers still know what each card is.
  ...
  function renderIdentity() {
    ...
    S.services.items.forEach(function (s) { grid.appendChild(el("span", null, s)); });
  }
  ...
  function renderAbout() {
    ...
    var items = $("#ab-items");
    (a.list || []).forEach(function (t) { items.appendChild(el("li", null, t)); });

    $("#ab-logosTitle").textContent = a.logosTitle;
    var logos = $("#ab-logos");
    (a.logos || []).forEach(function (t) { logos.appendChild(el("li", null, t)); });
  }
  ...
  var STICKERS = [
    { s: 111, rot: -8,  fill: "#8ed081", eye: [40, 62, 46, 8.5],
    ...
    { s: 116, rot: 6,   fill: "#5b9bf5", eye: [41, 62, 47, 8.5],
  ...
  function renderFooter() {
    ...
    STICKERS.forEach(function (st) {
      ...
      var lx = st.eye[0], rx = st.eye[1], ey = st.eye[2], er = st.eye[3];
      var pr = er * 0.42, po = er * 0.2;
      d.innerHTML =
        '<svg viewBox="0 0 100 100" aria-hidden="true">' +
        '<path d="' + st.d + '" fill="' + st.fill + '" stroke="#212121" stroke-width="4" stroke-linejoin="round"/>' +
        '<circle cx="' + lx + '" cy="' + ey + '" r="' + er + '" fill="#fff" stroke="#212121" stroke-width="3.4"/>' +
        '<circle cx="' + rx + '" cy="' + ey + '" r="' + er + '" fill="#fff" stroke="#212121" stroke-width="3.4"/>' +
        '<circle cx="' + (lx + po) + '" cy="' + (ey + po) + '" r="' + pr + '" fill="#212121"/>' +
        '<circle cx="' + (rx + po) + '" cy="' + (ey + po) + '" r="' + pr + '" fill="#212121"/>' +
        "</svg>";
      box.appendChild(d);
    });
    ...
    function tick() {
      ...
      clock.textContent =
        g("hour") + ":" + g("minute") + " " + g("dayPeriod").toUpperCase() +
  ...
  function open(group) {
    if (openThing === group) { closeOpen(); return; }
    closeOpen();
  ...
  function wirePanels() {
    Array.prototype.forEach.call(document.querySelectorAll(".pillgroup"), function (group) {
      group.querySelector(".pill").addEventListener("click", function (e) {
        e.stopPropagation();
        open(group);
      });
      group.querySelector(".panel").addEventListener("click", function (e) { e.stopPropagation(); });
    });
    ...
    $(".contactpop").addEventListener("click", function (e) { e.stopPropagation(); });
    ...
    document.addEventListener("pointerdown", function (e) {
      if (!openThing) return;
      if (e.target.closest && e.target.closest(".panel, .contactpop, .pill, .cta")) return;
      closeOpen();
    });
  ...
  function wireContactForm() {
    ...
    if (toggle) {
      toggle.addEventListener("click", function () {
        ...
        $(".cp-toggle__label").textContent = showingCall ? "Book a Call instead" : "Use the form instead";
      });
    }
  ...
  function wireCopyEmail() {
    ...
    btn.addEventListener("click", function () {
      var done = function () {
        ...
        clearTimeout(timer);
        timer = setTimeout(function () { btn.classList.remove("is-copied"); }, 1400);
      };
