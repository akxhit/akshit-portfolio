...
(function () {
  ...
  function initLenis() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (target) { e.preventDefault(); lenis.scrollTo(target, { offset: -24 }); }
    });
  ...
  function initIntro() {
    ...
    if (name && window.SplitText) {
      ...
      tl.from(name, { y: 26, opacity: 0, duration: .8 }, 0);
    }

    tl.from(".badge", { y: 12, opacity: 0, duration: .6, stagger: .07 }, 0.15)
      // .pill, not .pillgroup — the group is the panel's positioning
      // parent, and must stay transform-free
      .from(".pill", { y: -10, opacity: 0, duration: .6, stagger: .08 }, 0.1)
      .from(".tagline > *", { y: 14, opacity: 0, duration: .6, stagger: .045 }, 0.45)
      .from(".services", { y: 18, opacity: 0, duration: .7 }, 0.6)
      .from(".ctawrap, .contactrow", { y: 18, opacity: 0, duration: .7, stagger: .08 }, 0.68);
  ...
  function initCards() {
    ...
    gsap.utils.toArray(".card").forEach(function (card) {
      ...
      function through() {
        return { trigger: card, start: "top bottom", end: "bottom top", scrub: true };
      }
  ...
  function initMagnetic() {
    ...
    Array.prototype.forEach.call(targets, function (elm) {
      ...
      elm.addEventListener("pointerleave", function () { xTo(0); yTo(0); });
    });
  }
  ...
  function initStickers() {
    ...
    if (window.ScrollTrigger) {
      gsap.from(stickers, {
        y: 40, opacity: 0, scale: .8, duration: .8, ease: EASE,
    ...
    Draggable.create(stickers, {
      ...
      onDragStart: function () { this.target.classList.add("is-held"); },
      onRelease: function () {
        this.target.classList.remove("is-held");
        gsap.to(this.target, { scale: 1, duration: .35, ease: EASE });
      },
  ...
  function initTagline() {
    gsap.utils.toArray(".hw").forEach(function (hw) {
      ...
      hw.addEventListener("pointerenter", function () {
        gsap.fromTo(art,
          { opacity: 0, y: 14, scale: .8, rotate: -10 },
          { opacity: 1, y: 0, scale: 1, rotate: -4, duration: .5, ease: EASE });
      });
      hw.addEventListener("pointerleave", function () {
        gsap.to(art, { opacity: 0, y: 10, scale: .86, duration: .25, ease: "power2.in" });
      });
    });
  }
