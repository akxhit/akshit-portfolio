...
(function () {
  ...
  function init() {
    ...
    pill.querySelector(".nekopill__label").textContent = cfg.label || "You like cats?";
    ...
    function measureHome() {
      ...
      if (!started) { pos.x = home.x; pos.y = home.y; started = true; }
    }
    ...
    function setFollowing(next) {
      ...
      if (next) { disturbed = false; calmTicks = 0; }
      if (cfg.remember !== false) {
        try { localStorage.setItem(STORAGE_KEY, String(next)); } catch (e) { /* noop */ }
      }
    }
    ...
    function step() {
      ...
      if (shouldFlee) {
        ...
        calmTicks += 1;
        if (calmTicks >= RETURN_HOME_TICKS) { disturbed = false; calmTicks = 0; }
      }
      ...
      if (!following && fleeTicks > 0) {
        ...
        if (dist > 0.001 && dist > stopAt) {
          ...
          if (overrideTicks > 0) {
            ...
            behavior = "idle"; frame = 0;
          } else if (idleTime < 20) {
            behavior = "alert"; frame = 0;
          } else if (idleTime < 30) {
            behavior = "tired"; frame = 0;
          } else {
            var r = Math.random();
            if (!following && cfg.idleBehavior === "sit") {
              behavior = "idle"; overrideTicks = 2;
            } else if (r < 0.06) {
              behavior = "scratchSelf"; overrideTicks = 9;
            } else if (r < 0.08) {
              var walls = ["scratchWallN", "scratchWallS", "scratchWallE", "scratchWallW"];
    ...
    sw.addEventListener("click", function () { setFollowing(!following); });
